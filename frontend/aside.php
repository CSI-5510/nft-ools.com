...
<aside class="w-64">
   ...
   <ul class="flex flex-col relative h-screen w-full space-y-2">
      <li class="my-px">
		 ...
		 <?php if(!User::getUsername(USER_ID)): ?>
		 Account
		 <?php else: ?>
		 Hi <?php echo User::getFullName();?>.
		 <?php endif; ?>
		 </span>
      </li>
      <li class="my-px space-y-2">
         ...
         <span class="ml-3 text-gray-500">
               ...
               ?>			
            <a href=" <?php if($GLOBALS['config']['url_root']!== ""){echo $GLOBALS['config']['url_root'];echo"/";} echo $GLOBALS["url_loc"][0]; ?>/<?php echo User::isLoggedin() ? "logout" : "login"; ?>" class="<?php echo $GLOBALS["url_loc"][1] !== "login" ? "flex flex-row items-center h-10 px-3 rounded-lg text-gray-300 hover:bg-gray-100 hover:text-gray-700 transition" : "flex flex-row items-center h-10 px-3 rounded-lg text-gray-600 bg-gray-100 hover:bg-gray-100 hover:text-gray-700 "; ?>">
            <span class="flex items-center justify-center text-lg text-red-400">
               <svg
                  fill="none"
                  stroke-linecap="round"
      ...
      <?php if (User::isLoggedin()): ?>
         <li class="my-px">
            <a href="<?php if($GLOBALS['config']['url_root']!== ""){echo $GLOBALS['config']['url_root'];echo"/";} echo $GLOBALS["url_loc"][0]; ?>/orders" class="<?php echo $GLOBALS["url_loc"][1] !==
            "checkout"
      ...
      ?>
          ...
          <li class="my-px">
              <a href="<?php if($GLOBALS['config']['url_root']!== ""){echo $GLOBALS['config']['url_root'];echo"/";} echo $GLOBALS["url_loc"][0]; ?>/profile" class="<?php echo $GLOBALS["url_loc"][1] !==
              "Profile"
                  ? "flex flex-row items-center h-10 px-3 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition"
                  : "flex flex-row items-center h-10 px-3 rounded-lg text-gray-600 bg-gray-100 hover:bg-gray-100 hover:text-gray-700"; ?>">
               <span class="flex items-center justify-center text-lg text-gray-400">
      ...
      >  
         ...
         <div x-show="categoryCollapse" x-transition.opacity>
            <?php foreach ($category_list as $category): ?>
               <li class="my-px space-y-2">
                  <a href="<?php if($GLOBALS['config']['url_root']!== ""){echo $GLOBALS['config']['url_root'];echo"/";} echo $GLOBALS["url_loc"][0]; ?>/category/<?php echo $category["cat_id"]; ?>" class="<?php echo $GLOBALS["url_loc"][2] === $category["cat_id"] ? "flex flex-row items-center h-10 px-3 rounded-lg text-gray-600 bg-gray-100 transition hover:bg-gray-300 hover:text-gray-300" : "flex flex-row items-center h-10 px-3 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition ";?>">
                     <span class="ml-3 text-gray-500">
